        ...
        <section v-if="currentStep === 2" class="mt-5 rounded-lg border border-white/[.08] bg-white/[.025] p-5 md:p-6">
          ...
          <div class="mt-5 grid gap-4">
            ...
            <label class="block">
              ...
              <textarea v-model="form.brief" rows="7" class="mt-2 w-full resize-y rounded-lg border border-white/[.08] bg-black/20 px-4 py-3 text-sm leading-6 text-white outline-none transition focus:border-blue-400/55 focus:bg-black/30"></textarea>
            </label>
          </div>
        </section>
      </template>
      ...
      <div class="fixed bottom-0 left-0 right-0 z-20 border-t border-white/10 bg-[#0B0B0D]/92 px-5 py-4 backdrop-blur lg:left-[252px]">
        <div class="mx-auto flex max-w-[960px] items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <button v-if="!isLastStep && !loading" class="h-11 rounded-lg bg-bluebrand px-6 text-sm font-semibold text-white shadow-blue transition hover:bg-[#2D78FF] disabled:opacity-45" type="button" :disabled="!canContinue" @click="nextStep">
              Lanjut
            </button>
            <button v-else-if="!loading" class="h-11 rounded-lg bg-bluebrand px-6 text-sm font-semibold text-white shadow-blue transition hover:bg-[#2D78FF] disabled:opacity-60" type="submit" :disabled="saving">
