...
const createCampaign = async () => {
  ...
  try {
    await store.createAdminCampaign({
      ...form.value,
      ...
      <div class="mt-7 grid gap-3 md:grid-cols-3">
        ...
        >
          <span
            ...
            :class="index === currentStep ? 'bg-bluebrand text-white' : index < currentStep ? 'bg-emerald-400/14 text-emerald-100' : 'bg-white/[.055] text-white/44'"
          >
            {{ index + 1 }}
          </span>
          <span>
      ...
      <section v-if="currentStep === 2" class="mt-5 rounded-lg border border-white/[.08] bg-white/[.025] p-5 md:p-6">
        ...
        <div class="mt-5 grid gap-4">
          ...
          <label class="block">
            ...
            <textarea v-model="form.brief" rows="7" class="mt-2 w-full resize-y rounded-lg border border-white/[.08] bg-black/20 px-4 py-3 text-sm leading-6 text-white outline-none transition placeholder:text-white/24 focus:border-blue-400/55 focus:bg-black/30" placeholder="Tulis arahan campaign, gaya video, larangan, atau asset yang perlu dipakai."></textarea>
          </label>
        </div>
      </section>
